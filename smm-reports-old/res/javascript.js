...
$( document ).ready(function() {
  $(".dateRangeSaveSend").html("From: <input type='text' id='from_datepicker' class='datepicker'/>" +
                "To: <input type='text' class='datepicker' id='to_datepicker'/>&nbsp;<button id='find1'>Go</button>");
            $(".datepicker").datepicker({ dateFormat: "yy-mm-dd" });
});
...
function generate() {
    ...
    var str2 = "";
     ...
     var select_date = $('#select_date').val();
      ...
      var choose_report = $('#choose_report').val();

     
    var date = new Date();
    var dateToday = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate();
    var dateYesterday = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+(date.getDate()-1);
    var thisweekMon = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+(date.getDay()+ 9);
    var thisweekSun = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+(date.getDay()+15);
    var lastweekMon = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+(date.getDay()+2);
    var lastweekSun = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+(date.getDay()+8); 
    var from_datepicker = $("#from_datepicker").val();
     var to_datepicker = $("#to_datepicker").val();
 
 

    if(choose_report == 'save_n_send'){
       if(select_date == 'Date Range'){
          window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+from_datepicker+'&date_to='+to_datepicker, '_blank');
       } 
      else if(select_date == 'Today'){
         window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+dateToday+'&date_to='+dateToday, '_blank');
       } 
      else if(select_date == 'Yesterday'){
         window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+dateYesterday+'&date_to='+dateYesterday, '_blank');
       }
      else if(select_date == 'This Week'){
         window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+thisweekMon+'&date_to='+thisweekSun, '_blank');
       
       }
      else if(select_date == 'Last Week'){
         window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+lastweekMon+'&date_to='+lastweekSun, '_blank');
       
       }
      // else if(select_date == 'This Month'){
         // window.open('save_send.php?hierarchy_tree_id='+select_dept+'&date_from='+from_datepicker+'&date_to='+to_datepicker, '_blank');
    ...
    var var_is_mktg_workflows = "no";
    var is_checked = "";
    if (department == "mktg_workflows") {
        var_is_mktg_workflows = "yes";
        department = new Array;

        // if($('#include_mail_jet').prop('checked')){
        $("#select_sub_acc").find("input").each(function () {
            if ($(this)[0].id != "") {
                if ($(this).prop('checked')) {
                    if(is_checked != "no"){
                        is_checked = "yes";
    ...
    if(is_checked == "no"){
        $("#select_all_sub_acc").text("Select All");

    }
    console.log(department);
    if (var_is_mktg_workflows == "yes") {
        // $("#include_mail_jet").prop('checked');
        if (!department.length) {
            department = $("#mktg_workflows_all").val();
        }
    }
    ...
    xhr.queue({ //start xhr.queue1
        ...
        type: "POST",
        data: {
            is_mktg_workflows: var_is_mktg_workflows,
            generate: department,
                ...
                $.each(clients, function (htree_id) { //start each function
                    ...
                    $.each(clients[htree_id], function (index, element2) {
                        ...
                        $.each(element2, function (index2, element) {
                            ...
                            if ($("#select_dept").val() == "mktg_workflows") {
                                str += "data-htree='" + element.sub_account + "' data-val=\"" + element.project_name + "\"";
                                if (marketing_teams.hasOwnProperty(element.sub_account))
                                    str += ` data-htree_id="${marketing_teams[element.sub_account].dept}" data-team_id="${marketing_teams[element.sub_account].team}"`;
                                    // str += " data-htree_id='" + marketing_teams[element.sub_account].dept + "'";
                                else
                                    str += " data-htree_id='" + element.hierarchy_tree_id + "'";
                            } else {
                                str += "data-htree='" + element.node + "' data-val=\"" + element.project_name + "\"  data-htree_id='" + element.hierarchy_tree_id + "'";
                            }

                            // str += "data-all='sub_acc'";
                            ...
                            str += element.company + " - <span class='project_name " + class2 + "'>" + element.project_name +" "+ element.hierarchy_tree_id +  " test1231</span></label></div>";
                            if (i++ == 15) {
...
const snapShot = type => {
    ...
    let params = [];
    let department = $('#select_dept').val();
    if (type === 'view') {
        if (department === "all") {
            x = [];
            $("#select_dept").find("option").each(function () {
                if ($(this).val() != "all" && $(this).val() != "mktg_workflows") {                    
                    x.push($(this).val());
                }
            });
            department = x.join(",");
        ...
        $(".proj_ids").each(function() {
            ...
            if ($(this).is(":checked")) 
                params.push(
                    { 
                        ln_project_id   : $(this).val(), 
                        client_id       : $(this).attr("data-client-id"),
                        department      : $(this).attr("data-htree_id"),
                        dt_defined      : $('#select_date').val(),
                        date_start      : $('#from_datepicker').val(),
                        date_end        : $('#to_datepicker').val(),
                        dialer_workflow : $(this).attr("data-status"),     
    ...
    xhr.queue({
        type: 'POST',
        url: `/lead-nurturing/api/${type}-snapchat.php`,
        data: {
            params
        },
        success : result => type === 'insert' ? alert(result) : renderBreakdownTable(result)
    });

    if (type === 'insert') {

        department = $("#select_dept").val();
        if (department == "all") {
            department = new Array;
            $("#select_dept").find("option").each(function () {
                if ($(this).val() != "all" || $(this).val() != "mktg_workflows") {
        ...
        xhr.queue({
            ...
            success: xml => alert(`Campaign Stats ${xml}`)
        })
...
const renderBreakdownTable = data => {
    ...
    const headers = [
        data.date,
        'Campaigns', 'No. of Campaigns Enrolled in the Dialer', 'Number of Workflows', 'Workflows / Campaign',
        'Sent', 'Ave Sent / Campaign', 'Delivered', 'Delivered %', 'Opened', 'Opened %', 'Clicks', 'Clicks %',
        'CTR', 'Soft Bounce', 'Soft Bounce %', 'Hard Bounce', 'Hard Bounce %', 'Total Bounce', 'Total Bounce %',
        'Conversions', 'Conversions / Opens', 'Conversions / Delivered', 'Conversions triggered by Opens',
        'Conversions triggered by Opens %'
    ]

    const keys = [
        'dept', 'campaigns', 'dialer_workflow', 'workflows', 'avg_workflow', 'sent', 'avg_sent', 'delivered',
        'delivered_rate', 'opened', 'opened_rate', 'clicks', 'click_rate', 'ctr', 'soft_bounce', 'soft_bounce_rate',
        'hard_bounce', 'hard_bounce_rate', 'total_bounce', 'total_bounce_rate', 'conversions', 'conversions_opened_rate',
        'conversions_delivered_rate', 'opens_in_conversions', 'opens_in_conversions_rate'
    ];

    const table = new Table();
    
    table.thead('', 'style="font-size: 14px; white-space: nowrap; background: #34495E; color:#fff"');

    headers.forEach(i => table.th(i, '', 'style="color:#fff"'));

    data.report_dept.forEach(i => {
        table.tr('breakdown_dept_row');
        keys.forEach(x => table.td(i[x], 'b_table_td', 'style="padding: 10px; white-space: nowrap"'));
    });

    $("#companies").hide('slow');
    $('#breakdown_by_dept').html(table.getTable())    

}

const clientBreakdown = param => {

    const [hid, date_from, date_end] = param.split("_");

    const url = `client-breakdown-snapshot.php?hid=${hid}&date_from=${date_from}&date_to=${date_end}`;    

    popupCenter({url: url, title: 'Team / Client Breakdown', w: 1250, h: 900});  
...
const viewWeeklyReport = () => window.open('weekly-report.php', '_blank')
...
function clickFind() {
    ...
    var total_totalOpensConverted = 0;
        ...
        if ($(this).is(":checked")) {
            ...
            xhr.queue({  //start xhr.queue2
                ...
                success: function (xml) {
                    ...
                    if (dept_summary[htree] === undefined) dept_summary[htree] = { contactScheme: 0, sendMail: 0, clicks: 0, openedMails: 0, dynamicList: 0, callPriority: 0, reminder: 0, replies: 0, bounces: 0, softbounces: 0, hardbounces: 0, totalAction: 0, totalConverted: 0, totalOpensConverions: 0, schemeCount: 0 };
                    ...
                    totalOpensConverions = $(xml).find("total_opened_conversions").text();
                    ...
                    let event_states = []
                    if ($(xml).find("event_states").text() !== "")
                        event_states = JSON.parse($(xml).find("event_states").text())                                             

                    if (!conversions_breakdown.hasOwnProperty(htree))
                        conversions_breakdown[htree] = {};

                    if (event_states.length !== 0) {
                        for (let i in event_states) {
                            if (conversions_breakdown[htree].hasOwnProperty(i))
                                conversions_breakdown[htree][i] += event_states[i]
                            else
                                conversions_breakdown[htree][i] = event_states[i]
                        }
                    }
                    ...
                    if (totalOpensConverions == "") totalOpensConverions = 0;
                    ...
                    str += "<td class='as' style='text-align:center;'>" + totalOpensConverions + "</td>";
                    total_totalConverted += parseInt(totalConverted);
                    total_totalOpensConverted += parseInt(totalOpensConverions);
                    $(".total_act_log #overall_conv").text(total_totalConverted);
                    $(".total_act_log #overall_opens_conv").text(total_totalOpensConverted);

                    if (!converted_emails.hasOwnProperty(htree)) {
                        converted_emails[htree] = {}
                    }

                    if (convertedEmails.length !== 0) {
                        if (converted_emails[htree].hasOwnProperty(client_id)) {
                            converted_emails[htree][clients[client_id]].push(convertedEmails);
                        } else {
                            converted_emails[htree][clients[client_id]] = convertedEmails;
                        }
                    }
                    
                    // sessionStorage.setItem('converted', JSON.stringify(converted_emails));
                    sessionStorage.setItem('encoded', encodeURIComponent(window.btoa(JSON.stringify(converted_emails))));
                    ...
                    client_summary[htree][client_id].totalOpensConverions += parseInt(totalOpensConverions);
                    ...
                    dept_summary[htree].totalOpensConverions += parseInt(totalOpensConverions);
...
$(document).on("change", "#select_dept", function () {
    // alert($(this).val());

    if ($(this).val() == "mktg_workflows") {
        $("#select_sub_acc").show();
        $("#select_all_sub_acc").show();

        $("#mailjet_only").hide();
    } else {
        $("#select_sub_acc").hide();
        $("#select_all_sub_acc").hide();
        $("#mailjet_only").show();
    }
    // if()
});

$(document).on("change", ".sub_acc_input", function () {
    // alert($(this).val());
...
// $('#include_mail_jet').add('#include_mail_jet').on('change', function(){
//     $("#generate").click();
//     console.log($(this));
// });

$(document).on("click", ".include_mail_jet, .include_inactive_wf, .include_initial_blast", function () {
    $("#generate").click();
...
(function (window, undefined) {
    ...
    function end_loading() {
        ...
        if (queue.length == 0) {
            $(".bgload").hide(5000);   
            $("#overall_conv").hover(function() {
                $(this).css('cursor','pointer');
            })
            $("#overall_conv").click(function(e) {
                e.preventDefault();
                const url = `conversion_breakdown.php?params=${sessionStorage.getItem('encoded')}`;    
                // popupCenter({url: url, title: 'Conversion Breakdown', w: 850, h: 800}); 
                const param = { params : sessionStorage.getItem('encoded'), success_breakdown : encodeURIComponent(window.btoa(JSON.stringify(conversions_breakdown))) };
                openWindowWithPost('conversion_breakdown.php', "width=850, height=800, left=100, top=100", "NewFile", param);                
            });         
            // console.log(conversions_breakdown);
        }
...
function display_summary(client_summary, dept_summary, totalSend) {
    ...
    // hd += "<th style='width:8%;'>%</th>";
    hd += "<th style='width:8%;' title='Click through rate (Clicks/Delivered)'>CTR</th>";
    ...
    hd += "<th style='width:8%;'>Opens in Conversions</th>";
    ...
    for (var h in dept_summary) {
        ...
        for (var c in client_summary[h]) {
            ...
            const ctr = (client_summary[h][c].openedMails !== 0) ? (client_summary[h][c].clicks / delivered) * 100 : 0;
            let has_bounce = '';
            ...
            str += "<td class='td3' title='Click through rate (Clicks/Delivered)'>" + ctr.toFixed(2) + "%</td>";
            ...
            const ocRate = client_summary[h][c].totalOpensConverions !== 0 ? (client_summary[h][c].totalOpensConverions / client_summary[h][c].totalConverted) * 100 : 0;
            str += "<td class='td3' title='"+client_summary[h][c].totalOpensConverions+" out of "+client_summary[h][c].totalConverted+" conversion(s) had open history'>" + ocRate.toFixed(2) + "%</td>";
        ...
        const ctr = (dept_summary[h].openedMails !== 0) ? (dept_summary[h].clicks / delivered) * 100 : 0;
        ...
        str += "<th class='tl' title='Click through rate (Clicks/Delivered)'>" + ctr.toFixed(2) + "%</th>";
        ...
        const totalOcRate = dept_summary[h].totalOpensConverions !== 0 ? (dept_summary[h].totalOpensConverions / dept_summary[h].totalConverted) * 100 : 0;
        str += "<th class='tl' title='"+dept_summary[h].totalOpensConverions+" out of "+dept_summary[h].totalConverted+" conversion(s) had open history'>" + totalOcRate.toFixed(2) + "%</td>";
    ...
    for (var x in workflow_summary) {
        ...
        const ctr = (workflow_summary[x].opened_emails !== 0) ? (workflow_summary[x].clicks / delivered) * 100 : 0;
        ...
        ws += "<td title='Click through rate (Clicks/Delivered)'>" + ctr.toFixed(2) + "%</td>"
...
const exportBounces = (data, client_name) => {
    ...
    data.forEach(i => rows.push([
        i.first_name,
        i.last_name,
        ((i.company != null || i.company == '') ? i.company.replace(",", ";") : ''),
        i.email,
        i.country,
        i.bounce_type,
        ((i.bounce_error != null || i.bounce_error == '') ? i.bounce_error.replace(/,|#/g, ";") : '')
    ]));
    console.log(rows)
