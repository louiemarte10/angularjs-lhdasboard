        ...
        if($(this).is(":checked")){
            ...
            xhr.queue({  //start xhr.queue2
                ...
                success: function (xml) {
					...
					if (dept_summary[htree] === undefined) dept_summary[htree] = { contactScheme: 0, sendMail: 0, clicks: 0, openedMails: 0, dynamicList: 0, callPriority: 0, reminder: 0, replies: 0, totalAction: 0, totalConverted: 0, schemeCount: 0 };
                    ...
                    str += "<td class='as' style='text-align:center;'>"+ opens_new.toFixed(2)+"%</th>";
...
function display_summary(client_summary, dept_summary, totalSend) {
    ...
    hd +="<th style='width:8%;'>Replies </th>";
	...
	for(h in dept_summary){
		...
		for(c in client_summary[h]){			
            ...
            for (x in client_summary[h][c].workflow_summary) {
            ...
            str += "<td class='td3'>" + opensAvg.toFixed(2) + "%</td>";
			...
			str += "<td class='td3'>" + client_summary[h][c].replies + "</td>";
		...
		str += "<th class='tl'>" + opensAvg.toFixed(2) + "%</th>";
		...
		str += "<th class='tl'>" + dept_summary[h].replies + "</th>";
    ...
    ws += "<th>Bounces %</th>";
    ws += "</tr>";
    for (x in workflow_summary) {
       
        const totalSentAvg = (workflow_summary[x].email_sent / totalSend) * 100;
        const totalClicksAvg = (workflow_summary[x].opened_emails !== 0) ? (workflow_summary[x].clicks / workflow_summary[x].opened_emails) * 100 : 0;
        ...
        ws += "<td>" + opensAvg.toFixed(2) + "%</td>"
        ws += "<td>" + workflow_summary[x].clicks + "</td>"
        ws += "<td>" + totalClicksAvg.toFixed(2) + "%</td>"
        ws += "<td>" + workflow_summary[x].replies + "</td>"
